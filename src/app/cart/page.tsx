...
const Page = () => {
    ...
    // cart is saved in local storage in browser (client)
    // so serve doesn't know - has errors
    // to overcome this, check when component is mounted
                                    ...
                                    return (
                                        <li key={product.id} className="flex py-6 sm:py-10">
                                            <div className="flex-shrink-0">
                                                <div className="relative h-24 w-24">{typeof image !== "string" && image.url ? <Image fill src={image.url} alt="product image" className="h-full w-full rounded-md object-cover object-center sm:h-48 sm:w-48" /> : null}</div>
                                            </div>
                    ...
                    <section className="mt-16 rounded-lg bg-gray-50 px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8">
                        ...
                        <div className="mt-6 space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <p className="text-sm font-medium text-gray-900">{isMounted ? formatPrice(cartTotal) : <Loader className="h-4 w-4 animate-spin text-muted-foreground" />}</p>
                            </div>
                            ...
                            <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                                ...
                                <div className="text-sm font-medium-text-gray-900">{isMounted ? formatPrice(fee) : <Loader className="h-4 w-4 animate-spin text-muted-foreground" />}</div>
                            </div>

                            <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                                <div className="text-base font-medium text-gray-900">Order Total</div>
                                <div className="text-base font-medium text-gray-900">{isMounted ? formatPrice(cartTotal + fee) : <Loader className="h-4 w-4 animate-spin text-muted-foreground" />}</div>
                            </div>
                        </div>
